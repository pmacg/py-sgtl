...
def adjacency_spectrum(graph: sgtl.Graph, num_eigenvalues=None) -> List[float]:
    ...
    if num_eigenvalues >= graph.number_of_vertices() - 1:
        eigvals, _ = sp.linalg.eig(graph.adjacency_matrix().toarray())
    else:
        eigvals, _ = sp.sparse.linalg.eigs(graph.adjacency_matrix(), k=num_eigenvalues)
    return list(eigvals)
...
def laplacian_spectrum(graph: sgtl.Graph, num_eigenvalues=None, magnitude='smallest') -> List[float]:
    ...
    if num_eigenvalues >= graph.number_of_vertices() - 1:
        eigvals, _ = sp.linalg.eig(graph.laplacian_matrix().toarray())
    else:
        eigvals, _ = sp.sparse.linalg.eigs(graph.laplacian_matrix(), k=num_eigenvalues, which=mag_sp)
    return list(eigvals)
